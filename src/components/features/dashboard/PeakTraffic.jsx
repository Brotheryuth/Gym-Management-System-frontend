...
export default function PeakTraffic({
  ...
  return (
    <div className="purity-card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '16px 20px', height: '100%', justifyContent: 'space-between' }}>
      <div>
      ...
      <div style={{ display: 'flex', height: '135px', alignItems: 'flex-end', justifyContent: 'space-between', gap: '10px', padding: '10px 0 0 0', borderBottom: '1.5px solid var(--color-border)' }}>
        {slots.map((slot, idx) => {
          const count = slotCounts[idx] || 0;
          const heightPct = Math.max(10, (count / maxSlotCount) * 100);
          return (
            <div key={slot} className="traffic-bar-item" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
